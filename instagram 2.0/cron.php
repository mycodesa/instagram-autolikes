...
// Log dosyasını aç
$logfile = fopen('logdetail2.txt', 'a');
if (!$logfile) {
...
// send_reels_only değeri 0 olan tüm kullanıcıları al
$sql_users = "SELECT id, username, instagram_user_id, added_at FROM users WHERE send_reels_only = 0";
...
while ($row = $result_users->fetch_assoc()) {
    ...
    $sql_check = "SELECT last_checked FROM cron_log WHERE username='$username' AND send_reels_only=0";
    ...
    if ($result_check->num_rows == 0) {
        ...
        $sql_insert_log = "INSERT INTO cron_log (username, instagram_user_id, send_reels_only, last_checked) VALUES ('$username', '$instagram_user_id', 0, '$current_time')";
    ...
    // Kullanıcı gönderilerini almak için API çağrısı
    $posts_api_url = "https://api.instagapi.com/userposts/{$instagram_user_id}/10/";
    ...
    // API yanıtı hatalı mı kontrol et
    if (!isset($posts_data['data'])) {
        fwrite($logfile, "API Hatası: Kullanıcı: $username (Instagram ID: $instagram_user_id)\nAPI Yanıtı: " . print_r($posts_data, true) . "\n\n");
        continue; // Bir sonraki kullanıcıya geç
    }

    fwrite($logfile, "Kullanıcı: $username (Instagram ID: $instagram_user_id)\nAPI Yanıtı: " . print_r($posts_data, true) . "\n\n");
    ...
    // Son 10 gönderiyi al
    fwrite($logfile, "Kullanıcı: $username\nSon 10 Gönderi:\n");
    foreach ($posts_data['data']['items'] as $index => $post) {
        if ($index >= 10) break;
        if (!isset($post['taken_at'])) {
            fwrite($logfile, "Eksik 'taken_at' alanı: $username (Instagram ID: $instagram_user_id)\n");
            continue; // Bir sonraki gönderiye geç
        }
        $post_date = new DateTime('@' . $post['taken_at']);
        $post_date->setTimezone(new DateTimeZone('Europe/Istanbul'));
        $formatted_date = $post_date->format('Y-m-d H:i:s');
        fwrite($logfile, "$formatted_date - https://www.instagram.com/p/" . $post['code'] . "/\n");
    }
    ...
    foreach ($posts_data['data']['items'] as $post) {
        if (!isset($post['taken_at'])) {
            fwrite($logfile, "Eksik 'taken_at' alanı: $username (Instagram ID: $instagram_user_id)\n");
            continue; // Bir sonraki gnderiye geç
        }
        $post_date = new DateTime('@' . $post['taken_at']);
        $post_date->setTimezone(new DateTimeZone('Europe/Istanbul'));
        $formatted_date = $post_date->format('Y-m-d H:i:s');
        ...
        if ($formatted_date > $last_checked) {
            ...
            if ($conn->query($sql_insert_post) === TRUE) {
                fwrite($logfile, "Yeni gönderi eklendi: $post_link\n");

                // Kullanıcıya ait tüm SMM API bilgilerini al
    ...
    $current_time = (new DateTime('now', new DateTimeZone('Europe/Istanbul')))->format('Y-m-d H:i:s');
    $sql_update = "UPDATE cron_log SET last_checked='$current_time' WHERE username='$username' AND send_reels_only=0";
